...
def lookup_from_data(user_query):
    ...
    try:
        ...
        result = data_context_agent.generate_reply([{"role": "user", "content": user_query}])
        ...
        if relevant_data:
            ...
            interpretation_result = data_interpreter_agent.generate_reply([{"role": "user", "content": interpretation_query}])
            log_agent_invocation("data_interpreter", "Data interpretation completed", "Generated insights and recommendations")
...
def classify_query(user_query):
    ...
    try:
        ...
        decision = router_agent.generate_reply([{"role": "user", "content": user_query}])
        ...
        if classification not in ["lookup", "llm", "both"]:
            ...
            if any(keyword in user_query.lower() for keyword in ["what is my", "show me", "find", "gpa", "records", "data"]):
                classification = "lookup"
            elif any(keyword in user_query.lower() for keyword in ["how to", "improve", "advice", "help me", "suggest"]):
                classification = "llm"
            else:
                classification = "both"
...
def route_to_agents(user_query):
    ...
    try:
        ...
        result = selector_agent.generate_reply([{"role": "user", "content": user_query}])
        ...
        try:
            ...
            if any(keyword in user_query.lower() for keyword in ["academic", "study", "gpa", "course", "grade"]):
                ...
                selected = ["career"]
            elif any(keyword in user_query.lower() for keyword in ["stress", "mental", "health", "wellbeing", "welfare"]):
                selected = ["welfare"]
...
def get_single_agent_response(agent, user_query, context_data=None):
    ...
    try:
        ...
        response = agent.generate_reply([{"role": "user", "content": enhanced_query}])
...
def hybrid_response(user_query):
    ...
    if mode in ["lookup", "both"]:
        ...
        print(f"Data response: {data_response[:200] if data_response else 'None'}...")  # Debug
    
    # Get agent response if needed
    if mode in ["llm", "both"]:
        selected_agents = route_to_agents(user_query)
        print(f"Selected agents: {[getattr(agent, 'name', 'unknown') for agent in selected_agents]}")  # Debug
    ...
    if data_response and agent_response:
        if mode == "both":
            ...
            return agent_response  # Agent response should already include data context
    elif data_response:
        return data_response
    elif agent_response:
        return agent_response
    else:
        return "I apologize, but I couldn't find relevant information or generate a helpful response. Could you please try rephrasing your question or being more specific about what you'd like to know?"
